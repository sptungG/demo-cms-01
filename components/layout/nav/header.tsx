...
export const Header = ({ locale = "vn" }: Props) => {
  ...
  return (
    <header
      className={`bg-white sticky top-0 right-0 w-full sm:px-0 z-[99999] border-b backdrop-blur-xl transition-all duration-300`}
    >
      <nav
        data-state={menuState ? "active" : "inactive"}
        className={`w-screen m-auto px-4`}
      >
        <hr className="bg-vina-primary" />
        <div className={` ${isScrolled ? "bg-white shadow-sm" : "bg-white"}`}>
          <div className={`mx-auto container transition-all duration-300`}>
            <div className="relative duration-75 flex h-16 items-center justify-between py-3 lg:gap-0 lg:py-0">
              <Link
